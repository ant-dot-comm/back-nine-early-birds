...
create function public.member_leaderboard(p_mode text default 'back9')
returns table (
...
as $$
  ...
  szn as (
    ...
    where r.is_final and r.mode = p_mode and r.played_on >= date_trunc('year', current_date)
  ),
  round_totals as (
  ...
  select pr.id, pr.display_name, pr.first_name, pr.last_name, pr.initials,
    ...
    coalesce(ha.birdies, 0)::int, coalesce(ha.eagles, 0)::int, coalesce(ha.pars, 0)::int,
...
$$;
grant execute on function public.member_leaderboard(text) to authenticated;
