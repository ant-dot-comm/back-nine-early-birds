-- Lower secret odds to 1 in 50
create or replace function public.roll_secret_name()
returns text language plpgsql security definer set search_path = public as $$
declare v_uid uuid := auth.uid(); v_names text[]; v_name text; odds int := 50;
begin
...
-- Test accounts: real logins for previewing UI, hidden from real members.
...
create or replace view public.members as
  ...
  from public.profiles where not is_test;

-- Leaderboard excludes test users too.
